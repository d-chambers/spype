...
def func_with_kwargs(a: int, b: float = 2.) -> str:
    pass


def func_with_req_kwargs(a: int, *, b: float = 2., c: str = 'hey'):
    pass
...
class TestObjectIsType:
    ...
    is_type_tuple = (
        ...
        ([1., 2, 'bob'], Sequence[Union[int, float, str]]),
        ([1, 2, 3], Sequence),
        ({float: 1}, Mapping[type, int]),
        ({1: 1, 'bob': 2}, Dict[Union[int, str], int]),
        ({1: 2, 2: 3}, Any),
        ([Car(), Honda()], List[Car]),
        ((1, 2., Honda()), Tuple[int, float, Car]),
...
class TestCompatibleFunctions:
    ...
    compat_with_no_strict = (
        (union_output_1, union_func1),
    )
